...
var currentLang = 'tr';
try { currentLang = localStorage.getItem('efesoftlab-lang') || 'tr'; } catch (e) {}

// ===== Dil Değiştirme Fonksiyonu =====
function changeLanguage(lang) {
  currentLang = lang;
  try { localStorage.setItem('efesoftlab-lang', lang); } catch (e) {}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Preloader - sayfa hazır olunca kaldır
  var preloader = document.getElementById('preloader');
  if (preloader) { preloader.classList.add('hidden'); }

  // Yapılandırmayı DOM'a uygula
  if (typeof applyConfigToDOM === 'function') { applyConfigToDOM(siteConfig); }
  ...
  const currentTheme = (function() { try { return localStorage.getItem('efesoftlab-theme'); } catch(e) { return null; } })();
  ...
  themeToggle?.addEventListener('click', () => {
    ...
    if (isDark) {
      ...
      try { localStorage.setItem('efesoftlab-theme', 'light'); } catch (e) {}
      if (themeIcon) {
      ...
      document.documentElement.setAttribute('data-theme', 'dark');
      try { localStorage.setItem('efesoftlab-theme', 'dark'); } catch (e) {}
      if (themeIcon) {
        themeIcon.classList.remove('fa-moon');
        themeIcon.classList.add('fa-sun');
      }
  ...
  if (cookieConsent) {
    ...
    try { cookieAccepted = localStorage.getItem('efesoftlab-cookie'); } catch (e) {}
    if (!cookieAccepted) {
      setTimeout(() => {
  ...
  cookieAcceptBtn?.addEventListener('click', () => {
    try { localStorage.setItem('efesoftlab-cookie', 'true'); } catch (e) {}
    cookieConsent.classList.remove('show');
    setTimeout(() => { if (cookieConsent) cookieConsent.style.display = 'none'; }, 500);
  });

  // ===== Hoş Geldin Bildirimi =====
  var welcomeShown = false;
  try { welcomeShown = sessionStorage.getItem('efesoftlab-welcome'); } catch (e) {}
  if (!welcomeShown) {
    setTimeout(() => {
      showToast(
      ...
      try { sessionStorage.setItem('efesoftlab-welcome', 'true'); } catch (e) {}
    }, 2000);
  }
